...
def getFilterPropertyXml(propertyInfo, propertyName):
    ...
    if isinstance(propertyValue, dict):
      # A dictionary that contains 'type' and 'default' keys overrides
      # any inferred propertyType with 'type'
      if propertyValue.has_key('type') and propertyValue.has_key('default'):
        numberOfElements = propertyValue['length'] if propertyValue.has_key('length') else 1
        propertyDoc = propertyValue['doc'] if propertyValue.has_key('doc') else ''
        if propertyValue['type'] == 'file':
          propertyType = file
        elif propertyValue['type'] == 'float':
          propertyType = float
          rangeSpec = ''
          if propertyValue.has_key('min'):
            rangeSpec += ' min="%s"' % str(propertyValue['min'])
          if propertyValue.has_key('max'):
            rangeSpec += ' max="%s"' % str(propertyValue['max'])
          propertyDomain = """<DoubleRangeDomain %s name="range" />""" % rangeSpec
        elif propertyValue['type'] == 'int':
          propertyType = int
          rangeSpec = ''
          if propertyValue.has_key('min'):
            rangeSpec += ' min="%s"' % str(propertyValue['min'])
          if propertyValue.has_key('max'):
            rangeSpec += ' max="%s"' % str(propertyValue['max'])
          propertyDomain = """<IntRangeDomain %s name="range" />""" % rangeSpec
        elif propertyValue['type'] == 'bool':
          propertyType = bool
        elif propertyValue['type'] == 'string':
          propertyType = str
          if propertyValue.has_key('enums'):
            propertyDomain = """<StringListDomain name="%s">\n%s</StringListDomain>""" % (propertyName.lower(), '\n'.join(['<String value="%s"/>' % x for x in propertyValue['enums']]))
          else:
            propertyDomain = ''
        elif propertyValue['type'] == 'list':
          propertyType = list
        defaultValues = propertyValue['default']
    ...
    if 'propertyType' not in locals():
      print 'Failed to find type for %s' % propertyName
    ...
    if propertyType is bool:
      ...
      </IntVectorProperty>''' % (propertyName, propertyLabel, propertyName, defaultValues, numberOfElements, propertyDoc)
    ...
    if propertyType is int:
      ...
      <IntVectorProperty
        ...
        %s
        <Documentation>%s</Documentation>
      </IntVectorProperty>''' % (propertyName, propertyLabel, propertyName, defaultValues, numberOfElements, propertyDomain, propertyDoc)
    ...
    if propertyType is float:
      ...
      <DoubleVectorProperty
        ...
        %s
        <Documentation>%s</Documentation>
      </DoubleVectorProperty>''' % (propertyName, propertyLabel, propertyName, defaultValues, numberOfElements, propertyDomain, propertyDoc)
    ...
    if propertyType is str:
      ...
      <StringVectorProperty
        ...
        %s
        <Documentation>%s</Documentation>
      </StringVectorProperty>''' % (propertyName, propertyLabel, propertyName, defaultValues, numberOfElements, propertyDomain, propertyDoc)

    if propertyType is file:
        return '''
      <StringVectorProperty
        name="%s"
        label="%s"
      ...
      </StringVectorProperty>''' % (propertyName, propertyLabel, propertyName, defaultValues, numberOfElements, propertyDoc)
...
def getFilterPropertiesXml(info):
    ...
    if type(propertyInfo) == tuple or type(propertyInfo) == list:
      propertyDict = {x['name']:x for x in propertyInfo}
      order = [x['name'] for x in propertyInfo]
    else: # Must be a dictionary; make UI order alphabetical
      propertyDict = propertyInfo
      order = sorted(propertyDict.keys())

    xml = [getFilterPropertyXml(propertyDict, name) for name in order]
    return '\n\n'.join(xml)
